...
class BinaryTree {
    ...
    heightImplementation = (node) => {
        ...
        return 1 + Math.max(this.heightImplementation(node.leftChild), this.heightImplementation(node.rightChild))
    }
    ...
    isBinarySearchTree = () => {
        return isBinarySearchTreeImplemetation(this.rootNode, Number.MIN_VALUE, Number.MAX_VALUE)
    }

    isBinarySearchTreeImplemetation = (node, min, max) => {
        if(!node) return true
        if(node.value < min || node.value > max) return false
        return this.isBinarySearchTreeImplemetation(node.leftChild, min, node.value - 1 ) 
